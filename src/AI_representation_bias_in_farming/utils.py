...
def get_prompt(country, farm_type, generation_type):
    ...
    if (type1 == "reality"):
        cur_prompt = "Please create an image that accurately represents the reality of what most " + farm_type + " farms look like"
    elif (type1 == "90percent"):
        cur_prompt = "Please create an image that accurately represents what 90%% of " + farm_type + " farms look like"
    elif (type1 == "typical"):
        cur_prompt = "A typical " + farm_type + " farm"
    elif (type1 == "default"):
        cur_prompt = "Please create an image of a " + farm_type + " farm"
    elif (type1 == "basic"):
    ...
    if (revise_note == "no_revise"):
        cur_prompt = cur_prompt + "I NEED to test how the tool works with extremely simple prompts. DO NOT add any detail, just use it AS-IS:"
...
def gen_image_train(client, country, farm_type, generation_type, start_index, n):
    ...
    for img_count in range(start_index, (start_index+n)):
        ...
        revised_input = response.data[0].revised_prompt # get what GPT-4o automatically rephrased the prompt into
        ...
        save_megadata(img_count, country, farm_type, generation_type, cur_prompt, revised_input)
...
def save_imag(generation_type, img_count, farm_type, image_bytes, country):
    ...
    # define output dir
    img_dir = "./results/images/" + generation_type + "/"   
    if not os.path.exists(img_dir): 
        os.makedirs(img_dir) # create the directory if the directory does not exist

    # define filename
    if (pd.isna(country)):
        file_name = farm_type + "_farm_" + generation_type + "_" + str(img_count) + ".png"
    else:
        file_name = farm_type + "_farm_" + generation_type + "_" + country + "_" + str(img_count) + ".png"
...
def save_megadata(img_count, country, farm_type, generation_type, cur_prompt, revised_input):
    """
    Save the generated images into the local folder
    ...
    # define output dir
    data_output_dir = "./results/megadata/"
