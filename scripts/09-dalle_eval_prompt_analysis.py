...
def main():
    print(
        "\nBegin the analysis of prompts OpenAI used to evaluate DALL-E 3 for prompt following. The prompts are shared on their GitHub, and originally from MSCOCO.\n"
    )
    ...
    pig_prompts = pig_prompts[
        ~pig_prompts["prompts"].str.contains("|".join(pig_neg_terms))
    ]
    ...
    cattle_outdoor_prompts = cattle_outdoor_prompts[
        ~cattle_outdoor_prompts["prompts"].str.contains("|".join(indoor_terms))
    ]
    cattle_indoor_prompts = cattle_prompts[
        cattle_prompts["prompts"].str.contains("|".join(indoor_terms))
    ]
    cattle_indoor_prompts = cattle_indoor_prompts[
        ~cattle_indoor_prompts["prompts"].str.contains("|".join(outdoor_terms))
    ]
    cattle_other_prompts = cattle_prompts[
        ~cattle_prompts["prompts"].str.contains("|".join(outdoor_terms + indoor_terms))
    ]
    ...
    print(
        f"{len(cattle_outdoor_prompts)} prompts are describing cows outdoor on pasture, beach etc."
    )
    print(
        f"{len(cattle_indoor_prompts)} prompts are describing cows housed indoors, inside of pens and stalls."
    )
    print(
        f"{len(cattle_other_prompts)} prompts are describing cows housed indoors, inside of pens and stalls."
    )
