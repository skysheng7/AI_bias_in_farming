...
def main():
    ...
    no_revise_df = sub_df.copy()
    # Now modify the copy
    no_revise_df["cleaned_revised_prompt"] = no_revise_df["revised_prompt"].apply(
        clean_prompt
    )

    # Step 3: Select specific columns (corrected syntax)
    no_revise_df = no_revise_df[
        ["generation_type", "country", "farm_type", "prompt", "cleaned_revised_prompt"]
    ]
